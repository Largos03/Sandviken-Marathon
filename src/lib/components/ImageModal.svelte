...
{#if open}
	...
	>
		...
		>
			...
			<button
				on:click={closeModal}
				class="absolute top-2 right-2 z-20 rounded-full bg-white/90 p-2 text-gray-700 shadow-lg transition-colors hover:bg-white hover:text-black"
			...
			<div class="absolute top-2 left-2 z-20 flex flex-col gap-2">
				<button
					...
					class="rounded-full bg-white/90 p-2 text-gray-700 shadow-lg transition-colors hover:bg-white hover:text-black disabled:cursor-not-allowed disabled:opacity-50"
				...
				<button
					...
					disabled={zoomLevel === 1}
					class="rounded-full bg-white/90 p-2 text-gray-700 shadow-lg transition-colors hover:bg-white hover:text-black disabled:cursor-not-allowed disabled:opacity-50"
					aria-label={t('resetZoomAria')}
					title={t('resetZoomAria')}
				>
			...
			{#if zoomLevel > 1}
				<div
					class="absolute bottom-2 left-2 z-20 rounded-full bg-white/90 px-3 py-1 text-sm font-medium text-gray-700 shadow-lg"
				>
			...
			<!-- Help text -->
			<div
				class="absolute right-2 bottom-2 z-20 rounded-lg bg-white/90 px-3 py-1 text-xs text-gray-600 shadow-lg"
			>
				{#if zoomLevel > 1}
			...
			<div
				class="flex h-[80vh] w-full items-center justify-center overflow-hidden bg-gray-100 cursor-{zoomLevel >
				1
					? isDragging
						? 'grabbing'
						: 'grab'
			...
			>
				{#if imageSrc}
					...
					/>
				{:else}
					<p id="image-modal-title" class="text-center text-lg font-semibold">
						{t('imageNotAvailable')}
					</p>
				{/if}
			</div>
